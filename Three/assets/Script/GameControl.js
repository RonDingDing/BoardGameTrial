...
cc.Class({
    ...
    exchangeIcon: function(anchor) {
        var oneIcon = this.iconsDataTable[this.chooseIconPos.x][
    ...
    exchangeBack: function(anchor) {
        var oneIconData = this.iconsDataTable[this.chooseIconPos.x][
            this.chooseIconPos.y
        ];
        var oneIconItem = this.iconsTable[this.chooseIconPos.x][
            this.chooseIconPos.y
        ];
        var anotherIconData;
        var anotherIconItem;
        ...
        oneIconItem.runAction(
            cc.sequence(cc.moveTo(0.5, pos2.x, pos2.y), finished)
        );
    ...
    handleMessage: function(message, data) {
        if (message == "exchange") {
            ...
            for (var i = 1; i < this.row; i++) {
                for (var j = 1; j < this.col; j++) {
                    if (
                        this.iconsDataTable[i][j].state == ICON_STATE_CANCELED
                    ) {
                        ...
                        if (j != this.col) {
                            for (var k = j + 1; k < this.col; k++) {
                                ...
                                if (
                                    this.iconsDataTable[i][k].state !=
                                    ICON_STATE_CANCELED
                                ) {
                                    this.iconsDataTable[i][
                                        k
                                    ].state = ICON_STATE_CANCELED;
                                    isFind = true;
                                    this.iconsDataTable[i][
                                        j
                                    ].iconType = this.iconsDataTable[i][
                                        k
                                    ].iconType;
                                    break;
                                }
                        ...
                        if (!isFind) {
                            this.iconsDataTable[i][
                                j
                            ].iconType = this.getNewIconType(i, j);
                        }
                    }
                }
            }
            this.handleMessage("move");
        } else if (message == "move") {
            var finished = cc.callFunc(function(target) {
                this.moveNum = this.moveNum - 1;
                if (this.moveNum == 0) {
                    this.handleMessage("check");
                }
            }, this);
            for (var i = 1; i < this.row; i++) {
                for (var j = 1; j < this.col; j++) {
                    if (this.iconsDataTable[i][j].state == ICON_STATE_MOVE) {
                        ...
                        this.iconsTable[i][j].runAction(
                            cc.sequence(
                                cc.moveTo(0.1 * num, pos.x, pos.y),
                                finished
                            )
                        );
                    }
        ...
        for (var i = 1; i < this.row; i++) {
            for (var j = 1; j < this.col; j++) {
                if (
                    ...
                    this.deltaPos.x =
                        this.iconsTable[i][j].getPosition().x - touchLoc.x;
                    this.deltaPos.y =
                        this.iconsTable[i][j].getPosition().y - touchLoc.y;
    ...
    onmTouchMove: function(event) {
        if (this.isControl) {
            ...
            if (this.chooseIconPos.x == 1 && anchor == 1) {
                this.iconsTable[this.chooseIconPos.x][
                    this.chooseIconPos.y
                ].setPosition(
                    this.iconsPosTable[this.chooseIconPos.x][
                        this.chooseIconPos.y
                    ]
                );
                this.iconsTable[this.chooseIconPos.x][
            ...
            if (deltaDistance > 4900) {
                ...
                this.iconsTable[this.chooseIconPos.x][
                    ...
                    cc.v2(
                        touchLoc.x + this.deltaPos.x,
                        touchLoc.y + this.deltaPos.y
                    )
                );
            }
        }
    ...
    onmTouchEnd: function(event) {
        if (this.isControl) {
            ...
            this.isControl = false;
            this.handleMessage("exchange", { pos: touchLoc, anchor: anchor });
        }
    },
